...
export class Screens {
  ...
  levelIntro(config: LevelConfig, onGo: () => void): void {
    ...
    const obstacles = OBSTACLE_ORDER.filter((k) => config.obstacles[k] > 0)
      .map((k) => `<span>${OBSTACLE_EMOJI[k]} ${T.obstacleNames[k]} × ${config.obstacles[k]}${OBSTACLE_INTRO_LEVEL[k] === config.level ? ' <b>NEW</b>' : ''}</span>`)
      .join('');
    const card = this.show(`
  ...
  unlock(ids: SkillId[]): Promise<void> {
    return new Promise((resolve) => {
      const items = ids
        .map(
          (id) => `<div class="unlock-icon">${SKILL_EMOJI[id]}</div>
  ...
  progress(save: SaveData, onBack: () => void): void {
    const rows = NUMBERS.map((n) => {
      ...
      return `<tr><th class="${mastered ? 'mastered' : ''}">${mastered ? SKILL_EMOJI[n] : n}</th>${cells}
